...
| [`project-integration-architect`](project-integration-architect.md) | Onboards the catalog into an established repository while preserving its existing instructions, roles, skills, rules and design architecture. |
...
Specialists are intentionally narrow. They should not all be invoked for every UI task. The Project Integration Architect is an onboarding/migration role, while the methodology director is normally used sequentially before implementation specialists when a substantial UI concept is changing.
