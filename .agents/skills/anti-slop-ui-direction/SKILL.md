...
description: Use before substantial new or redesigned UI surfaces to derive a domain-specific interaction concept, reject generic AI/dashboard defaults, and emit a Design Direction Contract before implementation; skip trivial local fixes.
---

# Anti-Slop UI Direction

This skill is a **concept gate**, not a visual style guide and not a replacement for domain, Qt, interaction or audit skills.
...
- scientific/data visualization architecture;
- a high-frequency operator workflow;
- the relationship between map, plot, timeline, inspector or comparison views;
...
- existing `DESIGN.md` decisions when present;
...
Then identify the **primary work object**: the map field, atmospheric state, time interval, selected entity, comparison, document, image, etc. Controls are not the primary object unless controlling the system is itself the user's job.
...
Mentally substitute the domain data with an unrelated business domain. For meteorology, for example:

- temperature → revenue;
- pressure → conversion;
- precipitation → orders;
- forecast models → sales regions.

If the **primary work surface and interaction model** remain equally natural, the concept is probably generic and must be reconsidered.
...
Ask what information would make the interface materially misleading or unsafe if omitted, conflated or visually weakened. Domain truth outranks novelty. For meteorology this commonly includes valid time, observation/forecast distinction, model/run/cycle, units, source, freshness, missing data and uncertainty.

#### Implementation-reality test

Reject or adapt a concept that depends on fake interactions, brittle absolute positioning, web-only assumptions, inaccessible pointer-only behavior, unreasonable custom painting, or an implementation strategy that fights the target stack.
...
4. domain/Qt/motion specialists may refine implementation but must not silently replace the defining mechanism;
...
- Did implementation collapse back into cards + toolbar + combo + generic plots?
...
- Durable concept invariants go to `DESIGN.md`; rejected brainstorming does not.
...
- Saving all brainstorming and transient alternatives into `DESIGN.md`.
